...
export function installClaudeRules(cwd: string): Record<string, string> {
  ...
  for (const ruleId of manifest.rules) {
    ...
    if (!cfg) throw new Error(`Missing claude-code adapter entry for ${ruleId}`);
  ...
  const claudeMerged = mergeMarkedBlock(
    tryReadText(claudePath),
    claudeSpineContent(),
  );
