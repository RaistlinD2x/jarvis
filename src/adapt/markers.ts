...
export function mergeMarkedBlock(existing: string | null, inner: string): string {
...
export function stripMarkedBlock(existing: string): string | null {
  const next = existing.replace(BLOCK_RE, "").replace(/\n{3,}/g, "\n\n").trim();
  return next.length ? `${next}\n` : null;
}

export function agentsSpineContent(doctrine: string): string {
  return `${doctrine.trim()}\n`;
}

export function claudeSpineContent(): string {
  return `@AGENTS.md\n`;
}
