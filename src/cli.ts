...
program
  .name("jarvis")
  .description(
    "Inject metaplanning + Bar Raiser + Architect + Ponytail rules into Cursor and Claude Code",
  )
...
program
  ...
  .option(
    "--host <host>",
    "cursor | claude-code | all",
    "all",
  )
  .option("--scope <scope>", "project (user deferred)", "project")
  .action((opts: { host: string; scope: string }) => {
    ...
    console.log(
      `Installed pack ${lock.packVersion} for: ${lock.hosts.join(", ")}`,
    );
    for (const f of Object.keys(lock.files)) console.log(`  ${f}`);
  });
...
program
  ...
  .action((opts: { host?: string }) => {
    ...
    console.log(
      `Updated pack ${lock.packVersion} for: ${lock.hosts.join(", ")}`,
    );
  });
...
program
  ...
  .action(() => {
    ...
    if (!s.missing.length && !s.drift.length) console.log("No drift.");
  });
